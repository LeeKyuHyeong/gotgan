# 곳간(stock) — 운영 인프라 SSOT

> **곳간 프로젝트 범위만** 다룸 — 같은 서버(175.125.21.245)의 다른 프로젝트들은 각자의 server-infra 문서 보유.
> 추후 문서 간 중복 확인·최신화 후 통합 예정 (그래서 파일명 `-stock` 접미사).
> 앱 설계는 [`stock.md`](stock.md), 남은 작업은 [`TODO.md`](TODO.md) 참고.
